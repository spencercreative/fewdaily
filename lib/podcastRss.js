...
export const GetEpisode = (props) => {
	...
	useEffect(() => {
		...
		parser.parseURL(`https://anchor.fm/s/47ab7344/podcast/rss`, function ( err, feed ) {
			if (err) throw err;
			setFeed(feed);
		});
	}, []);
					...
					moment(item.isoDate)
						...
						>
							...
							>
								<iframe
									src={
										'https://anchor.fm/front-end-web-daily/embed/episodes/' +
										item.link.split('/episodes/')[1]
									}
                                    height='100%'
									width='100%'
									frameBorder='0'
									...
									title={'Podcast episode for ' + props.date}
								/>
							</div>
						</div>
					)
