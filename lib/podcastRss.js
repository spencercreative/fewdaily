...
export const GetEpisode = (props) => {
    ...
    return (
        <>
            {feed.items.map((item, i) => moment(item.isoDate).tz('America/New_York')._i.split('T')[0] == props.date &&
                    ...
                    <div className="border-t border-solid border-gray mt-10 md:mt-16 py-10 print:hidden" key={i}>
                        ...
                        <div id="podcast-episode" className="relative w-full h-0">
                            ...
                            <iframe src={'https://anchor.fm/fewdaily/embed/episodes/' + item.link.split("/episodes/")[1]} height="100%" width="100%" frameBorder="0" scrolling="no" className="absolute top-0 left-0 h-full w-full" title={'Podcast episode for ' + props.date} />
