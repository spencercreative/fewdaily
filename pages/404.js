...
const subheading = "It appears this page is missing. Please check the link or select a recent post below."

export default function Error({ allPosts }) {

    return (
        <MainLayout day={getCurrentDay()}>
            <MetaHead title={title} description={subheading} day={getCurrentDay()} />
            <h1 className="mt-0">{title}</h1>
            <p>{subheading}</p>
            <h3 className="mb-0">Recent Posts</h3>
            {allPosts.length > 0 && 
                <>
                <PostList posts={allPosts.slice(0,5)} count={allPosts.slice(0,5).length}/>
                {allPosts.length > 5 &&
