...
import { theNamedDay, splitSections, makeExcerpt } from 'lib/helpers'
            ...
            {router.isFallback ? (
                ...
                <MainLayout day={theNamedDay([post.slug])}>
                    <article id="assets">
                        <Head>
                            <title>
                                {post.title} Assets | {post.slug} | Front-End Web Daily
                            </title>
                            <meta name="description" content={makeExcerpt(post.content)} />
                        </Head>

                        <div className="text-center">
                            <Link href={'/' + post.slug}><a>Back to {post.title} from {post.slug}</a></Link>
                        </div>
                        ...
                        <div className="text-center">
                            <SocialImage post={post} width="500px" height="500px">
                                <h3 className="m-0 uppercase text-4xl my-6">{post.title}</h3>
                        ...
                        <div className="text-center">
                            <SocialImage post={post} width="360px" height="640px">
                                <h3 className="m-0 uppercase text-3xl my-6">{post.title}</h3>
                                {makeExcerpt(post.content).split(', ').map((element, index) =>
                        ...
                        <div className="text-center">
                            <SocialImage post={post} width="640px" height="360px">
                                <h3 className="m-0 uppercase text-3xl my-6">{post.title}</h3>
                                {makeExcerpt(post.content).split(', ').map((element, index) =>
                                    <h4 className="text-base mt-0 mb-3" key={index}>{element}</h4>
                                )}
